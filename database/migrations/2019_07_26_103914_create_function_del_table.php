...
class CreateFunctionDelTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('function_del', function (Blueprint $table) {
            ...
            $table->string('project_id', 64)->comment('项目ID');
            ...
            $table->dateTime('del_at')->comment('删除时间');
            $table->string('del_by', 64)->comment('删除人')->useCurrent();
        });

        DB::statement('ALTER TABLE `function_del` COMMENT "功能删除表"');

        Schema::table('functions', function (Blueprint $table) {
            $table->string('project_id', 64)->default('proid')->comment('项目ID')->after('parent_func_id');
        });
    ...
    {
        Schema::dropIfExists('function_del');
